...
sem_t EsvaziouPote;            // Sem�foro para sinalizar que urso esvaziou o pote
...
void LockMutex(pthread_mutex_t *Mutex, int thread_num) {
    ...
    status = pthread_mutex_lock(&FifoMutex);
    if (status != 0) {
    ...
    last_node_thread = new_node;
    fifo_size++;
    status = pthread_mutex_unlock(&FifoMutex);
    if (status != 0) {
        printf("Erro na liberacao do mutex! Codigo = %d\n", status);
        exit(0);
    }
...
int main() {
    ...
    status = pthread_mutex_init(&FifoMutex, &MutexAttr);
    if (status != 0) {
        printf("Erro na cria��o do mutex AcessaPote! Codigo = %d\n", status);
        exit(0);
    }
    ...
    status = sem_init(&EsvaziouPote, 0, 0);  // sempre retorna 0
    if (status != 0) {
        printf("Erro na inicializacao do semaforo! Codigo = %d\n", errno);
        exit(0);
    }
    ...
    for (i = 0; i < N_ABELHAS; i++) {
        ...
        if (status == 0)
            printf("Thread Abelha %d criada com Id= %0d \n", i, (int)&hThreads[i]);
        else {
    ...
    if (status == 0)
        printf("Thread Urso criada com Id= %0d \n", (int)&hThreads[N_ABELHAS]);
    else {
    ...
    for (i = 0; i < N_ABELHAS + 1; i++) {
        ...
        else if ((int)tRetStatus != -1)
            printf("Thread Abelha %d: status de retorno = %d\n", i, (int)tRetStatus);
        else
            printf("Thread Urso: status de retorno = %d\n", (int)tRetStatus);
    }
...
void *Thread_Urso(void *arg) { /* Thread representando o urso */
    ...
    int i = (int)arg;
    do {
        printf("Esperando o urso espreguicar\n");
        _getch();
